...
export default function CollaborationFaq({ lang = 'cs' }: { lang?: string }) {
  ...
  const questions = [
    ['experience', t('Potřebuji zkušenost?','Do I need experience?'), t('Ne. První setkání slouží i k tomu, abychom zjistili, kde právě jsi a co má smysl jako první.','No. The first session also helps us see where you are now and what makes sense as a first step.')],
    ...
    ['single', t('Můžu začít jedním tréninkem?','Can I start with a single session?'), t(`Ano. Jednorázový individuální trénink trvá ${single.minutes} minut a stojí ${formatTrainingPrice(single.priceCzk,language)}. Jedno setkání může stát samo o sobě.`, `Yes. A single one-to-one session lasts ${single.minutes} minutes and costs ${formatTrainingPrice(single.priceCzk,language)}. One session can stand on its own.`)],
  ];
