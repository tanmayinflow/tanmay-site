...
import { MobileDoubleLine, MobileHomeLines } from './MobileDoubleLine';
...
export default function MobileHome({lang}:{lang:string}) {
  ...
  return <>
    ...
    </div><MobileHomeLines/><p className="m-eyebrow m-home-anchors">{c('Opening',3)}</p></section>
    ...
    <section className="m-chapter m-home-about" aria-labelledby="m-about-home-title"><div className="m-inner"><div className="m-about-composition"><Photo asset="prague" alt={c('HomeAbout',3)} className="m-prague"/><div className="m-about-heading"><p className="m-eyebrow">Kryštof Švec</p><h2 id="m-about-home-title">{c('HomeAbout',1)}</h2><MobileVajra className="m-about-symbol"/></div></div><p className="m-about-bio m-bio-highlight-practice"><RichText>{c('HomeAbout',2)}</RichText></p><TextLink href={routePath('pribeh',lang)} arrow>{lang==='en'?'More about me':'Více o mně'}</TextLink><MobileDoubleLine/></div></section>
