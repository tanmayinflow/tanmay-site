...
export default function MobileHome({lang}:{lang:string}) {
  ...
  return <>
    ...
    <section className="m-chapter m-home-work" aria-labelledby="m-work-title"><div className="m-inner"><div className="m-work-layout"><div className="m-handstand-composition"><Photo asset="earthField" alt="" className="m-handstand-earth"/><Photo asset="handstand" alt={t('handstandAlt',lang)} className="m-handstand"/></div><div className="m-work-text"><h2 id="m-work-title">{c('HomeWork',0)}</h2><div className="m-work-captions">{[1,3,5].map(n=><StoryCard key={n} title={n===3?t('clearPriorities',lang):c('HomeWork',n)} lang={lang} className="m-work-caption"><p><RichText>{c('HomeWork',n+1)}</RichText></p></StoryCard>)}</div></div></div><TextLink href={routePath('praxe',lang)}>{lang==='en'?'The whole practice':'Více o praxi'}</TextLink></div></section>
    <Chapter title={c('HomeCollab',0)} className="m-home-formats"><div className="m-editorial-rail"><p><RichText>{c('HomeCollab',1)}</RichText></p><p><RichText>{c('HomeCollab',2)}</RichText></p></div><Rail title={t('collaboration',lang)} footerLink={<TextLink href={routePath('spoluprace',lang)}>{lang==='en'?'Ways to work together':'Možnosti spolupráce'}</TextLink>} items={approvedLists.PageSpoluprace.formats.map((r,i)=>[r[0],r[1],mobileFormatSummaries[i][0],r[3],mobileFormatSummaries[i][1]])} lang={lang}/></Chapter>
    <MobileReviews lang={lang}/>
    <section className="m-chapter m-home-about" aria-labelledby="m-about-home-title"><div className="m-inner"><div className="m-about-composition"><Photo asset="prague" alt={c('HomeAbout',3)} className="m-prague"/><div className="m-about-heading"><p className="m-eyebrow">Kryštof Švec</p><h2 id="m-about-home-title">{c('HomeAbout',1)}</h2></div></div><p className="m-about-bio m-bio-highlight-practice"><RichText>{c('HomeAbout',2)}</RichText></p><TextLink href={routePath('pribeh',lang)}>{lang==='en'?'More about me':'Více o mně'}</TextLink></div></section>
